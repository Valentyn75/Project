...
DetectorFactory.seed = 0  # Декларація детектора для стабільності
...
def LangDetect(text: str, set: str) -> str:
    ...
    set = “all” (по замовчуванню) – функція повертає мову і коефіцієнт довіри
    """
    try:
        detected_lang = detect(text)
        confidence = 1.0  # langdetect не надає точного коефіцієнта довіри
        if set == 'lang':
            return detected_lang
        elif set == 'confidence':
            return str(confidence)
        else:  # 'all' за замовчуванням
            return f"{detected_lang}, Confidence: {confidence}"
    except Exception as e:
...
def CodeLang(lang: str) -> str:
    ...
    """
    try:
        # Словник для перетворення
        lang_dict = {v: k for k, v in GoogleTranslator.get_supported_languages().items()}
